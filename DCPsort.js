...
async function main() {
  ...
  // Create input
  let inputSet = splitInput(
    generateInput(config.chunks * config.perChunk, config.maxIntGenerated),
    config.chunks
  );
  ...
  job.on("accepted", () => {
    ...
    console.log(
      `${
        Math.round((Date.now() - timeStart) / 100) / 10
      }s\tJob Accepted, beginning distributed sorting. Id: ${job.id}`
    );
  });
  ...
  console.log(
    `${
      Math.round((Date.now() - timeStart) / 100) / 10
    }s\tParallel sorting of chunks complete, beginning merging`
  );
  ...
  console.log(
    `${
      Math.round((Date.now() - timeStart) / 100) / 10
    }s\tMerging chunks complete. ${
      config.validateResults ? "Validating..." : "Skipping validation (config)"
    }`
  );
  ...
  if (config.validatedResults) {
    if (validateOrder(resultSet)) {
      console.log(
        `${
          Math.round((Date.now() - timeStart) / 100) / 10
        }s\tSort order validated. All done!`
      );
    } else
      console.error("ERROR: Numbers not properly sorted! Something is wrong.");
  }

  console.log(
    `~~~\nFINISHED\nNumbers:\t${config.chunks * config.perChunk}\tChunks:${
      config.chunks
    }\nAcceptance:\t${(timeAccepted - timeStart) / 1000}\nSorting:\t${
      (timeSorted - timeAccepted) / 1000
